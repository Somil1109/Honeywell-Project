...
def generate_normal_data(num_records):
    ...
    for i in range(num_records):
        # establishing stable per-entity habits
...
def inject_impossible_travel(start_time):
    # same entity, geographically distant IPs, implausible time gap
    return [
        {
...
def inject_lateral_movement(start_time):
    # expanding resource breadth unusually
    events = []
    for i in range(5):
        events.append({
...
def inject_device_spoofing(start_time):
    # returning entity_id with mismatched OS/MAC fingerprint
    return [{
...
def inject_low_and_slow(start_time):
    # gradual exfiltration over days
    events = []
    for i in range(10):
        events.append({
            'entity_id': 'ENT_140', 'entity_type': 'user', 
            'timestamp': start_time + timedelta(days=i, hours=2), # off-hours
